        ...
        <div class="col-xl-6">
            <div class="card">
                <div class="card-header align-items-center d-flex">
                    <h4 class="card-title mb-0 flex-grow-1">Create Merchant
                    </h4>
                </div><!-- end card header -->

                <div class="card-body">
                    <p class="text-muted">Create new Merchant
                    </p>
                    @if (session('success'))
                    ...
                    <div class="live-preview">
                        <form action="{{ route('manage-merchants.store') }}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-3">
                                ...
                                <select class="form-control @error('keeper_id') is-invalid @enderror" name="keeper_id"
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}"
                                            {{ old('keeper_id') == $user->id ? 'selected' : '' }}>
                                            {{ $user->name }}</option>
                                    @endforeach
                                </select>
                                @error('keeper_id')
