...
class MerchantController extends Controller
{
    ...
    {
        $merchants = $this->merchantService->create($request->validated());
        return response()->json(new MerchantResource($merchants), 201);
    }

    public function update(MerchantRequest $request, int $id)
    {
        try {
            $merchants = $this->merchantService->update($id, $request->validated());
            return response()->json(new MerchantResource($merchants));
        } catch (ModelNotFoundException $e) {
            return response()->json([
                'message' => 'merchant not found',
